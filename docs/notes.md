...
$h$ は接触集合の消滅時刻にほぼ影響しない（$h=0.5/1/1.5/2$ で $t_{\max}=0.2992/0.2990/0.2986/0.2980$）。
...
- $h$ には鈍感（上記）。$\alpha$ には**非単調**に依存（0.005→0.177, 0.01→0.299, 0.02→0.300, 0.05→0.184）
  ため、パラメータのずれでは説明できない。
