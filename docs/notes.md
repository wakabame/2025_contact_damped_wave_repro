...
論文 2 例の再現と独自例（Example 3）の実装・検証で分かったことの記録。数値はすべて本リポジトリの
...
$Q_{\mathrm{num}}$ はスキーム由来の数値散逸に対応する。$Q_{\mathrm{num}}$ は滑らかな解・$\varepsilon$ 固定では
$O(\Delta t)$ だが、$\Delta t/\varepsilon$ を固定したままでは衝突を通じて一様に消えるわけではなく、
**論文解像度でも全エネルギー損失の 14–18% を占める**（Ex.1: 18%, Ex.2: 14%）。ソルバーはこの 3 つを
...
- $1\le\Delta t/\varepsilon<2$: 減衰するが振動（跳ね返り）。線形安定ではあるものの、
  $\Delta t/\varepsilon=1$ でも接触集合は大きく劣化する（§4 の表）
- $\Delta t/\varepsilon\ge2$: 増幅し、エネルギーが増加して破綻
...
| $\min\eta^0$ | 1（$x=0.65$ の極小） | **−1**（$x=0.65$。$(0.5,0.8)$ で負、全体の 30% が最初から障害物の下） |
| 弾性エネルギー | 21.4 | **3729.0**（内部の不連続由来）|
| 端点クランプによる追加 | 0 | **+2499.0**（$\eta^0(0)=0\to h=1$ の強制）|
| $E(0)$（= 弾性 + 運動 749.8） | 771.2 | **6977.8**（9 倍）|
| 初回接触時刻 | 0.0256 | **0.0000**（$(0.5,0.8)$ で $\eta^0<0$、初めから接触） |
...
計画時に懸念したほど敏感ではなかった。既定は $\{\eta<0\}$（**貫入集合**。ペナルティ力の台を含むが
一致はしない — 力はさらに $v<0$ を要し、貫入ノードの約 85% は $v\ge0$ で力ゼロ）。
tol は `--contact-tol` で変更できる。
...
`results/convergence/convergence.txt`。論文と同じ設定での**自己収束**の確認であり、
格子掃引は $\Delta x=\Delta t$ を**同時に**細分し、$\varepsilon$ 掃引は格子固定で行う
（各掃引の最細メンバーが参照解）。時間・空間・ペナルティの誤差を分離するものではない
— それには独立な $\Delta x$/$\Delta t$ 掃引と $\Delta t/\varepsilon\to0$ の細分が要るが、
陽的ペナルティの制約 $\Delta t<\varepsilon$ が両者を結合している（§1.2）。

Example 1、$\varepsilon=5\times10^{-4}$ 固定:

| $\Delta x=\Delta t$ | $\Delta t/\varepsilon$ | 接触面積 | $\|\eta(T)-\eta_{\mathrm{ref}}\|_\infty$ | 備考 |
|---|---|---|---|---|
| $10^{-3}$ | 2.00 | 0.00099 | 8.50 | **不安定**（増幅） |
| $5\times10^{-4}$ | 1.00 | 0.0494 | $5.5\times10^{-2}$ | **非単調**（跳ね返り） |
...
| 3 | `contact_dissipation` は各ステップでは非負でないのに「散逸 ≥ 0」と表示していた | `contact_work` に改名、非負性の主張を撤回（時間積分は本リポジトリの全実行で正だが、符号の保証はない。§5.2 #3）|
...
1・2・5 に対する回帰テストを追加した。

### 5.2 第 2 回レビュー（2026-08-29、リポジトリ全体）

高深刻度の欠陥はなし（ステンシルと三重対角系の組み立ては論文 §6 と一致との判定）。
指摘 14 件（Medium 8 / Low 6）を以下のとおり対処した。

| # | 内容 | 対処 |
|---|---|---|
| 1 | 既定の「接触集合」$\{\eta<0\}$ をペナルティの作用集合と説明していたが、力は $v<0$ も要する（貫入ノードの約 85% は力ゼロ） | 貫入集合として説明し直し（docstring・§2 (e)）|
| 2 | 「$Q_{\mathrm{num}}$ は $O(\Delta t)$ で消える」は過大主張（論文解像度で全損失の 14–18%）。GIF の散逸曲線が粘性+接触のみだった | 主張を限定（§1.1）。GIF は粘性+接触+数値の完全収支を表示 |
| 3 | 接触仕事の時間積分に符号保証はないのに「正」と断言 | 「全実行で正（観察であり定理ではない）」に修正 |
| 4 | $\Delta t/\varepsilon\ge1$ を一律 UNSTABLE と表示（単調性と線形安定性の混同） | `is_penalty_monotone()`（< 1）と `is_penalty_linearly_stable()`（< 2）に分離。表示も NON-MONOTONE / UNSTABLE を区別 |
| 5 | 収束スクリプトが Δt・Δx・ε の分離収束を示すかのような記述 | 自己収束（$\Delta x=\Delta t$ 同時細分 + 格子固定 ε 掃引）と明記（§4）|
| 6 | 原文式 Ex.2 が $(0.5,0.8)$ で負（最小 −1、30% が最初から貫入）である点が未記載 | §2 (b)・README・docstring に追記、テストで固定 |
| 7 | `.npz` 名に `store_every` が含まれず上書きされ得る | ファイル名に `_se{store_every}` を追加（回帰テストつき）|
| 8 | (A2) のテストが空間和後のスカラーしか見ていない | スナップショットから $P^i$ と $Q_{\mathrm{con}}$ を独立再構成して照合するテストに置換 |
| 9 | 連結性テストの 2 つ目の assert がトートロジー | 斜め帯の合成マスクで 1 成分 vs $n$ 成分を厳密に検証。CLI スモークテストとステンシル残差テストも追加 |
| 10 | README が接触集合を「identical」と過大表現 | 「目視一致」に修正し、比較が目視であること・Ex.2 が推定初期データであることを明記 |
| 11 | 削除済み plan の「Phase」用語とテスト数 45 の残存 | 記述を更新 |
| 12 | 「2 例とも弦全体がほぼ一様に落下」「$v^0$ は端点で −50」は Ex.2 に当てはまらない | Ex.1（一様落下・1 成分）と Ex.2（右 40% は 100 倍遅い・2 成分）を区別、端点は「非零」に修正 |
| 13 | パッケージメタデータに URL がない（配布物内の README の相対リンク切れも指摘） | `[project.urls]` を追加。ライセンス表記は選択が必要なため未対応 |
| 14 | threshold モードの許容誤差が CLI から変えられない | `--contact-tol` を追加し summary・図・GIF に伝搬 |

## 6. Example 3（独自例）— 転がる接触前線

論文の 2 例では速く落ちる部分がほぼ同時に接触するため、接触集合は「三角形が縮んで消える」形になる
（Ex.1 は弦全体が一様に落ちて 1 成分、Ex.2 は右 40% が 100 倍遅く落ちて 2 成分）。検証済みの
ソルバーで、**接触の起こり方が別の型になる例**を 1 つ作った（`results/ex3/`）。
...
- **初期データが (1.3) と完全に整合する**: $\eta^0(0)=\eta^0(l)=h$ に加え $v^0(0)=v^0(l)=0$。
  論文の 2 例は $v^0$ が端点で非零のまま（Ex.1 は両端 $-50$、Ex.2 は左端 $-50$・右端 $-0.5$）
  不連続で、`solve()` が端点を 0 にクランプする際に端点に初期層ができる。
  Example 3 にはそれがない（$\sin(\pi x)$ 因子がその役割）。
- $|v^0|_{\max}=68.8$（$x\approx0.26$）で、$\Delta t/\varepsilon=0.4$ の単調減衰条件は満たしたまま。
...
3. エネルギー $E(t)$ と累積散逸（粘性 + 接触 + 数値。§1.1 のとおり数値散逸も損失の 14–18% を
   占めるので、完全な収支を表示する — 2 本の曲線が厳密に鏡像になる）。接触の瞬間に
   エネルギーが落ちる様子が見える。

生成物: `results/ex1/animation.gif`（1.5 MB）、`results/ex2/animation.gif`（1.4 MB）、
`results/ex3/animation.gif`（2.1 MB, 180 フレーム, 765×544）。
フレーム数は `Result` の保存済みスナップショットから間引くので、`--store-every` を大きくした
実行でもそのままアニメーションにできる。

## 8. 残っている作業

- 接触散逸が接触境界に集中すること（Thm 2.3）の数値的確認。
- 図の論文との並列比較画像の自動生成（現状は目視比較）。
- 時間・空間・ペナルティ誤差を分離した収束テスト（独立な $\Delta x$/$\Delta t$ 掃引と
  $\Delta t/\varepsilon\to0$ の $\varepsilon$ 細分。§4 の注記参照）。
- パッケージのライセンス表記（選択待ち）。
